...
- No port forwards, host directory sharing, synchronized clipboard, or automatic
  ...
  drifts.
- A clean instance may carry one further drive, `materials.iso`, generated by
  Sandfort from a file or folder the user chose. It is attached `ReadOnly`, as
  removable optical media so the desktop offers it, and `repairBundle` reasserts
  that on every state read. It is **never** attached to the setup VM or the
  protected baseline, and `repairBundle` removes both the drive entry and the
  file from those roles. The guest is handed an image built from a **copy**, so
  the reverse direction is impossible by construction rather than by
  configuration — it does not depend on `ReadOnly` reaching QEMU, on a plist key,
  or on anything UTM promises. What `ReadOnly` adds is that the guest cannot
  write to the copy either.
...
Materials do not change that advice, and it is worth separating the two
directions to see why. Sending a file in cannot expose anything else on the Mac:
the guest reads a copy Sandfort built, not the file, and has no route back to it.
What sending a file in does is put that file inside the sandbox, where anything
running there can read it. Send in a work file only if you accept that a hostile
program in that instance can read it, and never a credential, key, wallet, or
secret.
